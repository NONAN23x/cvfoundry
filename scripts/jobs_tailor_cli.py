...
def command_build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with OutputLock(output):
        ...
        try:
            with tempfile.TemporaryDirectory(prefix=".jobs-tailor-build-", dir=output.parent) as temporary_name:
                staging = Path(temporary_name)
                assembled_path = staging / "tailored-resume.json"
                _write_json(assembled_path, tailored)
                _write_json(staging / "tailoring-payload.json", payload)
                _write_json(staging / "effective-policy.json", policy)
                notes_path = output / "tailoring-notes.md"
                staged_notes_path = staging / "tailoring-notes.md"
                if notes_path.exists():
                    shutil.copy2(notes_path, staged_notes_path)
                else:
                    staged_notes_path.write_text(
...
def command_doctor(args: argparse.Namespace) -> dict[str, Any]:
    ...
    command = (
        ...
        else [sys.executable, "-m", "scripts.doctor"]
    )
    completed = subprocess.run(
